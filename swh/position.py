...
class PosMgrBase(object):
    ...
    def get_open_close(self, order_symbol, order_amount, order_direction, order_rule_flag):
        print(self.position[order_symbol])
        if order_rule_flag == "YES_TOD_FLAG":
            short_position = self.get_short_position(order_symbol)
            if order_direction == Direction.BUY:
                if short_position > 0:
                    if order_amount > short_position:
                        return [short_position, OpenClose.CLOSE]
                    elif order_amount <= short_position:
                        return [order_amount, OpenClose.CLOSE]
                else:
                    return [order_amount, OpenClose.OPEN]
            if order_direction == Direction.SELL:
                long_position = self.get_long_position(order_symbol)
                if long_position > 0:
                    if order_amount > long_position:
                        return [long_position, OpenClose.CLOSE]
                    elif order_amount <= long_position:
                        return [order_amount, OpenClose.CLOSE]
                else:
                    return [order_amount, OpenClose.OPEN]

        yes_position = self.get_yes_position(order_symbol)
        if order_rule_flag == "YES_TOD_SHFE_FLAG":
            short_position = self.get_short_position(order_symbol)
            if order_direction == Direction.BUY:
                if short_position > 0:
                    if order_amount > short_position:
                        if yes_position["short"]["pos"] == 0:
                            return [short_position, OpenClose.CLOSE]
                        elif yes_position["short"]["pos"] > 0 and order_amount > yes_position["short"]["pos"]:
                            return [yes_position["short"]["pos"], OpenClose.CLOSE_YES]
                        elif yes_position["short"]["pos"] > 0 and order_amount <= yes_position["short"]["pos"]:
                            return [order_amount, OpenClose.CLOSE_YES]
                    elif order_amount <= short_position:
                        if yes_position["short"]["pos"] == 0:
                            return [order_amount, OpenClose.CLOSE]
                        elif yes_position["short"]["pos"] > 0 and order_amount > yes_position["short"]["pos"]:
            ...
            if order_direction == Direction.SELL:
                ...
                if long_position > 0:
                    if order_amount > long_position:
                        if yes_position["long"]["pos"] == 0:
                            return [long_position, OpenClose.CLOSE]
                        elif yes_position["long"]["pos"] > 0 and order_amount > yes_position["long"]["pos"]:
                            return [yes_position["long"]["pos"], OpenClose.CLOSE_YES]
                        elif yes_position["long"]["pos"] > 0 and order_amount <= yes_position["long"]["pos"]:
                            return [order_amount, OpenClose.CLOSE_YES]
                    elif order_amount <= long_position:
                        if yes_position["long"]["pos"] == 0:
